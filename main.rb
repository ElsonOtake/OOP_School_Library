...
def list_all_books
  puts "Title: \"Lord of the rings\", Author: Tolkien"
  puts "Title: \"Game of thrones\", Author: George martin"
  puts
end
...
def create_a_person
  ...
  puts "Person created successfully #{age} #{name}"
  puts
end
...
def create_a_book
  ...
  begin
    ...
    puts "Book created successfully #{title} #{author}"
  puts
end
...
def main
  begin
    option = menu
    if %w(1 2 3 4 5 6 7).include?(option)
      case option
      when "1"
        list_all_books
      when "2"
      ...
      puts "Choose a correct option please!"
    end
  end until option == "7"
end
